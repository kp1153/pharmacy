...
function MedSearch({ index, item, medicines, onSelect }) {
  ...
  const substitutes =
    selectedMed && selectedMed.stock === 0 && selectedMed.generic
      ? medicines
          .filter(
            (m) =>
              m.id !== selectedMed.id &&
      ...
      {substitutes.length > 0 && (
        <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-xs font-bold text-amber-700 mb-1">
            ⚠️ Out of stock — Substitutes available:
          </p>
          {substitutes.map((s) => (
            <button
              key={s.id}
...
export default function NewSale() {
      ...
      prev.map((it, i) =>
        i === index
          ? {
              ...it,
              qty: parseInt(qty) || 1,
              amount: parseFloat(it.mrp || 0) * (parseInt(qty) || 1),
            }
          : it,
      ),
    );
  ...
  const hasNarcotic = items.some(
    (it) => it.scheduleType && it.scheduleType !== "general",
  );
  ...
  async function handleSubmit() {
    ...
    if (hasNarcotic && !doctorName) {
      alert("Scheduled drug है — Doctor name जरूरी है");
      return;
    }
    setSaving(true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-blue-700 px-4 py-4 flex items-center gap-3">
        <Link href="/dashboard/sales" className="text-blue-200 text-sm">
          ← Back
        </Link>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              window._patientName = e.target.value;
            }}
            className={inp}
          />
          <input
            ...
            placeholder="Phone"
            value={patient.phone}
            onChange={e => { setPatient({ ...patient, phone: e.target.value }); window._patientPhone = e.target.value; }}
            className={inp}
          />
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    value={item.batch}
                    className={inp + " bg-white text-gray-500 text-sm"}
                  />
                </div>
                <div>
          ...
          <button
            onClick={addRow}
            className="w-full border-2 border-dashed border-blue-300 text-blue-600 font-bold py-2 rounded-xl hover:bg-blue-50 transition"
          >
            + Add Medicine
          </button>
        </div>
        ...
        {hasNarcotic && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-3">
            <h2 className="font-bold text-red-700">
              ⚠️ Scheduled Drug — Extra Details Required
            </h2>
            <input
              type="text"
