...
export default function NewPurchase() {
  ...
  async function handleSubmit() {
    ...
    if (!validItems.length) { alert("कम से कम एक दवाई ज़रूरी है"); return; }
    setSaving(true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-blue-700 px-4 py-4 flex items-center gap-3">
        <Link href="/dashboard/purchases" className="text-blue-200 text-sm">← वापस</Link>
        <h1 className="text-white font-extrabold text-lg">📦 नई खरीद</h1>
      </div>
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          <h2 className="font-bold text-gray-700">वितरक की जानकारी</h2>
          <input type="text" placeholder="वितरक का नाम" value={supplierName} onChange={e => setSupplierName(e.target.value)} className={inp} />
          <input type="text" placeholder="इनवॉइस नंबर" value={invoiceNo} onChange={e => setInvoiceNo(e.target.value)} className={inp} />
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          {items.map((item, i) => (
            <div key={i} className="border border-gray-100 rounded-lg p-3 space-y-2">
              <input type="text" placeholder="दवाई का नाम *" value={item.medicineName} onChange={e => updateItem(i, "medicineName", e.target.value)} className={inp} />
              <div className="grid grid-cols-2 gap-2">
                <input type="text" placeholder="बैच" value={item.batch} onChange={e => updateItem(i, "batch", e.target.value)} className={inp} />
                <input type="text" placeholder="एक्सपायरी" value={item.expiry} onChange={e => updateItem(i, "expiry", e.target.value)} className={inp} />
                <input type="number" placeholder="मात्रा" value={item.qty} onChange={e => updateItem(i, "qty", e.target.value)} className={inp} />
                <input type="number" placeholder="खरीद मूल्य" value={item.purchasePrice} onChange={e => updateItem(i, "purchasePrice", e.target.value)} className={inp} />
                <input type="number" placeholder="एमआरपी" value={item.mrp} onChange={e => updateItem(i, "mrp", e.target.value)} className={inp} />
        ...
        <button onClick={handleSubmit} disabled={saving} className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl text-xl disabled:opacity-50">
          {saving ? "सेव हो रहा है..." : "खरीद सेव करो"}
        </button>
      </div>
    </div>
  );
